...
namespace DeepRim
{
	public class GenStep_ElevationFertility_Deep : GenStep
	{
		...
		public override void Generate(Map map, GenStepParams parms)
		{
			...
			ModuleBase input = new Perlin(0.0209999997168779, 2.0, 0.5, 6, Rand.Range(0, int.MaxValue), QualityMode.High);
			...
			if (random == Rot4.North)
			{
				...
				input2 = new Translate(0.0, 0.0, 0.0 - (double)map.Size.z, input2);
			}
			else if (random == Rot4.East)
			{
				input2 = new Translate(0.0 - (double)map.Size.x, 0.0, 0.0, input2);
			}
			...
			float b = (!map.TileInfo.WaterCovered) ? 3.402823E+38f : 0f;
